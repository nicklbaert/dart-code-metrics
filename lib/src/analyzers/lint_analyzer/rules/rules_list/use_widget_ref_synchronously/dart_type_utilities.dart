...
bool argumentsMatchParameters(
  ...
  for (final argument in arguments) {
    if (argument is NamedExpression) {
      final element = argument.expression.canonicalElement;
      if (element == null) {
        return false;
      }
      namedArguments[argument.name.label.name] = element;
    } else {
      final element = argument.canonicalElement;
      if (element == null) {
        return false;
      }
  ...
  for (var runIdx = 0; runIdx < positionalArguments.length; runIdx++) {
    if (positionalArguments[runIdx] != positionalParameters[runIdx]) {
      return false;
    }
  }
...
bool canonicalElementsFromIdentifiersAreEqual(
  ...
  if (rawExpression1 == null || rawExpression2 == null) {
    return false;
  }
  ...
  if (expression1 is PrefixedIdentifier) {
    return expression2 is PrefixedIdentifier &&
        canonicalElementsAreEqual(
          expression1.prefix.staticElement,
...
bool typesAreUnrelated(
  ...
  final promotedLeftType = typeSystem.promoteToNonNull(leftType);
  final promotedRightType = typeSystem.promoteToNonNull(rightType);
...
bool _isFunctionTypeUnrelatedToType(_, DartType type2) {
...
extension on TypeSystem {
  bool interfaceTypesAreUnrelated(
    ...
    if (leftElement == rightElement) {
      ...
      final leftTypeArguments = leftType.typeArguments;
      final rightTypeArguments = rightType.typeArguments;
      ...
      final sameSupertypes = leftElement.supertype == rightElement.supertype;
  ...
  DartType get typeForInterfaceCheck {
    ...
    if (self is TypeParameterType) {
      if (self is TypeParameterTypeImpl) {
        final promotedType = self.promotedBound;
